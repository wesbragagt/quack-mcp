...
describe('QuackMCPServer', () => {
  ...
  describe('loadCSV', () => {
    ...
    it('should load CSV file successfully with mocked file system', async (t) => {
      const server = new QuackMCPServer();
      
      // Mock the database query execution
      t.mock.method(server as any, 'executeQuery', async (query: string) => {
        if (query.includes('CREATE OR REPLACE TABLE')) {
          return [];
        }
      ...
      const result = await server.loadCSV({ 
        file_path: './data/credit-card.csv', // Use actual file that exists
        table_name: 'test_table'
      });

      assert.ok(result.content);
      assert.strictEqual(result.content.length, 1);
      assert.ok(result.content[0].text.includes('Successfully loaded CSV'));
      assert.ok(result.content[0].text.includes('test_table'));
    });
  });
