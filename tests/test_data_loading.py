...
from quack_mcp.exceptions import CSVLoadError, FileDiscoveryError
...
class TestDataLoadingTools:
    ...
    @pytest.mark.asyncio
    async def test_load_multiple_csvs_with_file_list(self, server, create_multiple_csv_files):
    ...
    @pytest.mark.asyncio
    async def test_load_multiple_csvs_with_pattern(self, server, create_multiple_csv_files):
        """Test loading multiple CSVs with glob pattern."""
        tools = DataLoadingTools(server)
        
        pattern = str(Path(list(create_multiple_csv_files.values())[0]).parent / "*.csv")
        
        args = LoadMultipleCSVsArgs(
            pattern_or_files=pattern,
            union_by_name=True,
            include_filename=True
        )
        result = await tools.load_multiple_csvs(args)
...
class TestDataLoadingIntegration:
    ...
    async def test_load_and_query_csv(self, server, create_csv_file):
        ...
        query_result = await server.execute_query("SELECT COUNT(*) as count FROM integration_test")
    ...
    async def test_load_multiple_files_different_schemas(self, server, temp_dir):
        ...
        query_result = await server.execute_query("SELECT COUNT(*) as count FROM union_test")
    ...
    async def test_nested_directory_discovery(self, server, create_nested_csv_files):
        ...
        root_dir = Path(list(create_nested_csv_files.values())[0]).parent
